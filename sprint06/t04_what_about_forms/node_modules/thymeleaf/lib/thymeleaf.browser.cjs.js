...
class AttrAttributeProcessor extends AttributeProcessor {
  ...
  constructor(prefix, isomorphic) {
    super(prefix, NAME$e, isomorphic);
  }
  /**
  ...
  process(element, attribute, attributeValue, context) {
    ...
    if (/(.+=.+,)*.+=.+/.test(attributeValue)) {
      attributeValue.split(',').forEach(attribute => {
        ...
        element.setAttribute(name, stringUtils.escapeHtml(typeof processorResult === 'string' ? processorResult : JSON.stringify(processorResult)));
      });
    }
...
function deserialize(htmlString) {
  /* istanbul ignore if */
  {
    return domUtils.deserialize(htmlString);
  }
}
/**
...
function serialize(documentFragment) {
  /* istanbul ignore if */
  {
    return domUtils.serialize(documentFragment);
  }
}
...
async function extractFragment(dialectPrefix, fragmentInfo, context) {
  ...
  if (templateResolver) {
    ...
    return dumbQuerySelector.$(`[${dialectPrefix}\\:${NAME$9}^="${fragmentName}"]`, template) || dumbQuerySelector.$(`[data-${dialectPrefix}-${NAME$9}^="${fragmentName}"]`, template);
  }
...
class InsertAttributeProcessor extends AttributeProcessor {
  ...
  async process(element, attribute, attributeValue, context) {
    ...
    if (fragmentInfo) {
      ...
      if (fragment) {
        let fragmentSignature = getThymeleafAttributeValue(fragment, this.prefix, NAME$9);
        let {
          parameterNames
...
class UnlessAttributeProcessor extends AttributeProcessor {
  ...
  process(element, attribute, attributeValue, context) {
    ...
    if (expressionResult) {
      domUtils.clearChildren(element); // TODO: element.remove()?
...
class WithAttributeProcessor extends AttributeProcessor {
   ...
   */
  constructor(prefix, isomorphic) {
    super(prefix, NAME$1, isomorphic);
  }
  /**
...
new ThymeleafRule('StringConcatenation', Sequence('Concatenatable', OneOrMore(Sequence(/\+/, 'Concatenatable'))), values => context => {
  ...
  return arrayUtils.flatten(matchers).reduce((curr, acc) => {
...
class TemplateEngine {
  ...
  processFile(filePath, context = {}) {
    /* global "browser" */
    return Promise.reject(new Error('Cannot use TemplateEngine.processFile() inside a browser')) ;
  }
  /**
...
exports.ThymeleafExpressionLanguage = ThymeleafExpressionLanguage;
